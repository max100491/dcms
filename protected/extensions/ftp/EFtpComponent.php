...
class EFtpComponent extends CApplicationComponent
{
    ...
    public function connect()
    {
        if($this->_connection === null){
            ...
            // Check connection
            if(!$this->_connection)
                throw new CException('FTP Library Error: Не удалось установить соединение!');

            // Check login
            if((empty($this->username) AND empty($this->password)) AND !$login_result)
                throw new CException('FTP Library Error: Войти не удалось!');
	...
	public function close()
	{
        if ($this->getActive()){
            ...
            throw new CDbException('EFtpComponent неактивно и не может выполнять никаких FTP операций.');
        }
	}
	...
	public function setOptions($config)
	{
        if($this->getActive()){
            if(!is_array($config))
                throw new CException('EFtpComponent Error: Параметры конфигураций должены быть массивом!');
            ...
            foreach($config as $key => $value)
            {
                ...
                if(!ftp_set_option($this->_connection, $key, $value))
                    throw new CException('EFtpComponent Error: Система не сможет установить опцию FTP: "'.$key.'" со значением: "'.$value.'"');
            }

            return $this;
        }
        else{
            throw new CDbException('EFtpComponent неактивна и не может выполнять никаких FTP операции.');
        }
	}
    ...
    public function getFilesFolder($dir)
    {
        $pant = Yii::getPathOfAlias('application.modules').'/';
        foreach ($dir as $dirFile) {
            $path = $pant.str_replace('/dcms_modules/', '', $dirFile);
            if(@$this->chdir($dirFile)){
                @mkdir($path);
                $dirFile = $this->listFiles($dirFile);
                $this->getFilesFolder($dirFile);
            }else{
                $this->get($path, $dirFile);
            }
        }
    }
